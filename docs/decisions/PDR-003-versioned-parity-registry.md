...
The compatibility harness keeps an append-only, tracked parity registry for every robot jar and team. Classic Robocode is the behavioural reference. Each observation records the source identity, official setup, engine artifacts, outcomes, normalized errors, and the later diagnosis or repair that changed its status.
