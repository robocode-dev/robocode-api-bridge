...
- Asterisks are stripped from the supplied name **first**.
- `..` anywhere in the *stripped* name is then rejected with a `java.security.AccessControlException` ("no relative path allowed") — classic's own `AdvancedRobotProxy.getDataFile` throws the same exception for the same reason, so a robot that already handles it on classic behaves the same way here. Classic itself checks `..` on the raw name *before* its `RobotFileSystemManager.getDataFile` strips asterisks, which lets a name like `".*."` collapse into `".."` after passing the check — a real escape in classic. This design deliberately checks the stripped name instead: `C-005`'s safety rationale (untrusted jars, run unmodified) permits the bridge to be stricter than classic here, and no legitimate robot's behavior depends on encoding a traversal past an asterisk.
