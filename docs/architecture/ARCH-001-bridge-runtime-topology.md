...
Team support (`CAP-006`) is the first real test of this shape. A team jar is several robots plus a `.team` descriptor, and classic archives can package those member robots in nested jars, so the wrapper's one-jar-to-one-bot-directory assumption had to give: it stages each nested member jar and emits a team boot-entry directory alongside each member's, naming them for the Tank Royale booter's own team-grouping mechanism.
